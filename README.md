...
- [Установка](#установка)
- [Развертывание и проверка Kubernetes](#развертывание-и-проверка-Kubernetes)
...
## Развертывание и проверка Kubernetes

### 1. Применение конфигурации и развертывание ресурсов
Для развертывания всех сервисов и компонентов выполните следующую команду:
...
После развертывания проверьте состояние всех подов в пространстве имен `app-namespace`:

```bash
kubectl get pods -n app-namespace
```

Ожидаемый результат для подов: статус `Running`. Если поды находятся в статусе `CrashLoopBackOff`, продолжите к следующему шагу.

### 3. Логи подов
Чтобы получить информацию о проблемах с подом, например `auth-service`, используйте команду:

```bash
kubectl logs <pod-name> -n app-namespace
```

Замените `<pod-name>` на реальный ID пода.

### 4. Проверка ресурсов пода
Если проблема сохраняется, используйте команду для получения детальной информации:
...
### 5. Удаление ресурсов
Чтобы удалить все ресурсы, выполните:

```bash
kubectl delete -f setup.yaml
```
Если пространство имен не удаляется, можно принудительно завершить процесс удаления:

```bash
kubectl get namespace app-namespace -o json | jq '.spec = {"finalizers":[]}' | kubectl replace --raw "/api/v1/namespaces/app-namespace/finalize" -f -
```
### 6. Horizontal Pod Autoscaler (HPA)
Для проверки состояния HPA используйте:

```bash
kubectl get hpa -n app-namespace
```

### 7. Портфорвардинг для локального тестирования
Чтобы протестировать `auth-service` локально, выполните:

```bash
kubectl port-forward svc/auth-service -n app-namespace 8080:8080
```

После этого доступ к сервису можно получить по адресу: `http://localhost:8080`.
...
Разработано [tigor7750@gmail.com](mailto:tigor7750@gmail.com).
